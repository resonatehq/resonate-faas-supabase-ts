...
import { type AnyFunc, Core as AsyncCore } from "@resonatehq/sdk/async";
...
/**
 * Serverless Resonate client backed by the async/await execution engine.
 *
 * Functions are written as ordinary `async` functions using `await` (see
 * `@resonatehq/sdk/async`). This class is the worker side of the shim: each HTTP
 * invocation carries a single `execute` message which it drives through
 * {@link AsyncCore} to either completion or suspension. It does not maintain a
 * network recv loop or heartbeat -- serverless functions are invoked per message
 * -- so it builds a fresh {@link SupabaseNetwork} and {@link AsyncCore} per request.
 */
export class Resonate {
  ...
  private ttl: number;
  // ponytail: test seam — inject an in-memory network's send (LocalNetwork)
  // to drive the handler without Postgres. Undefined in prod → SupabaseNetwork.
  private send?: Send;
  ...
  /**
   * Registers an async function with Resonate for execution and version control.
   */
